...
async function testApi() {
  ...
  try {
    ...
    try {
      const products = await client.getList(ENDPOINTS.product, new URLSearchParams({ limit: '5' }));
    ...
    try {
      const stock = await client.getList(ENDPOINTS.stockAll, new URLSearchParams({ limit: '5' }));
    ...
    try {
      const counterparties = await client.getList(ENDPOINTS.counterparty, new URLSearchParams({ limit: '5' }));
    ...
    try {
      ...
      orders.rows?.slice(0, 5).forEach((o: { name: string; sum?: number; moment?: string }) => {
    ...
    try {
      const orgs = await client.getList(ENDPOINTS.organization, new URLSearchParams({ limit: '5' }));
    ...
    console.log('🏭 Склады...');
    try {
      const stores = await client.getList(ENDPOINTS.store, new URLSearchParams({ limit: '5' }));
